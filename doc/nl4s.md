...
0x00d  | 1    | `0000 0000` | Reserved (always zero) ?
0x00e  | 1    | `pppp pppp` | p=program_no(0-98)
0x00f  | 1    | `0000 0000` | Reserved (always zero) ?
0x010  | 1    | `---- ----` | ?
...
0x131  | 1    | `-rr- ----` | r=reverb_type(0-3)[^12]
0x132  | 1    | `--x- ttta` | x=fx_on(0-1); t=fx_type(0-5)[^13]; a=fx_amount(0-127)
0x133  | 1    | `aaaa aarb` | a=^; r=reverb_on(0-1); b=reverb_bright(0-127)
0x134  | 1    | `bbbb bb--` | b=^
0x135  | 1    | `---- -eee` | e=delay_tempo(0-127)
0x136  | 1    | `eeee ----` | e=^

[^1]: LFO1 Shape mapping:  
    `000` (`0x0`) = square  
    `001` (`0x1`) = fast rising  
    `010` (`0x2`) = fast falling  
    `011` (`0x3`) = slow falling  
    `100` (`0x4`) = slow rising  
    `101` (`0x5`) = triangle

[^2]: LFO1 Destination mapping:  
    `000` (`0x0`) = filter  
    `001` (`0x1`) = OSC MOD  
    `010` (`0x2`) = OSC MIX  
    `011` (`0x3`) = OSC2 
...
[^7]: Not all values are valid. See wave table in `nl4_wavetable.ods`  

[^8]: OSC1 Mod Type mapping:  
    `000` (`0x0`) = mod off  
    `001` (`0x1`) = FM1  
    `010` (`0x2`) = FM2  
    `011` (`0x3`) = FM3  
